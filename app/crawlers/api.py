...
from app.crawlers.spiders.zhihu_hot_spider import  crawl_zhihu_hot
# 导入京东商品爬虫函数（新增，用于通用化扩展）
from app.crawlers.spiders.jd_product_spider import crawl_jd_products

# 2. 创建路由组
router = APIRouter(prefix="/crawlers", tags=["pach"])
...
@router.post("crawl_jd_products")
def trigger_jd_products(url:str,db:Session = Depends(get_db)):
    crawled_count = crawl_jd_products(url,db)
    return {"message":f"成功{crawled_count}", "source_url": url}

@router.post("/crawl_zhihu_hot")
def trigger_zhihu_hot(url: str = "https://www.zhihu.com/hot", db: Session = Depends(get_db)):
    crawled_count = crawl_zhihu_hot(url, db)
    return {"message": f"成功爬取 {crawled_count} 条知乎热榜数据", "source_url": url}